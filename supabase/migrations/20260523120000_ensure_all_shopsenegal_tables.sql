-- ShopSenegal — schéma complet idempotent
-- Exécuter dans Supabase → SQL Editor ou : supabase db push
-- Tables : public.drivers, public.places, public.users, public.orders
...
-- Colonnes ajoutées après coup (migration douce si la table orders existait déjà)
alter table public.orders add column if not exists client text;
alter table public.orders add column if not exists telephone text;
alter table public.orders add column if not exists adresse text;
alter table public.orders add column if not exists note text;
alter table public.orders add column if not exists creneau text;
alter table public.orders add column if not exists paiement text;
alter table public.orders add column if not exists besoins jsonb;
alter table public.orders add column if not exists photos integer;
...
alter table public.orders add column if not exists paydunya_invoice_token text;
alter table public.orders add column if not exists estimated_total_fcfa integer;
...
-- Recharge le schéma vu par l’API REST
